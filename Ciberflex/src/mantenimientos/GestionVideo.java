...
public class GestionVideo implements VideoInterface{
	...
	public Video obtener(int id) {
		...
		try {
		   con = MySQLConexion.getConexion(); 
		   String sql = "select * from 	VIDEOS where ID_VIDEO = ?"; // sentencia sql
	...
	public int editar(Video v) {
		...
		try{
			con = MySQLConexion.getConexion();
			String sql = "update VIDEOS set TITULO_VIDEO=?, TEMPORADA_VIDEO=?, DESCRIPCION_VIDEO=?, URL_VIDEO=?, URL_IMAGEN_VIDEO=?, ESTADO=? where ID_VIDEO = ?";
			
			pst = con.prepareStatement(sql);
			pst.setInt(1, v.getId_contenido());
			pst.setString(1, v.getTitulo_video());
			if(v.getTemporada_video() == 0) pst.setString(2, null);
			else pst.setInt(2, v.getTemporada_video());
			pst.setString(3, v.getDescripcion_video());
			pst.setString(4, v.getUrl_video());
			pst.setString(5, v.getUrl_imagen_video());
			pst.setInt(6, v.getEstado());
			pst.setInt(7, v.getId_video());
